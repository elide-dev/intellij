 ...
 */
package elide.tooling.project.manifest.web

import elide.tooling.project.manifest.web.Browsers.Companion.defaults
...
sealed interface Browsers {
   ...
   */
  operator fun contains(spec: String): Boolean = browser(spec).let { parsed ->
    parsed in this
  }
   ...
   */
  @Serializable
  sealed interface Browser : Browsers

  /**
   * Default implementation of a [Browser] instance, which just represents the spec via a string.
   */
  @Serializable
  data class DefaultBrowser internal constructor(private val token: String) : Browser {
   ...
   */
  @Serializable
  sealed interface BrowserList : Browsers

  /**
   * Default implementation of a [BrowserList] instance, which holds more than one [Browser].
   */
  @Serializable
  data class DefaultBrowserList internal constructor(
  ...
  interface BrowserListBuilder {
     ...
     * @return This builder, for chaining.
     */
    fun specify(browser: String): BrowserListBuilder = when (',' in browser) {
   ...
   */
  class DefaultBrowserListBuilder internal constructor(
  ...
  companion object {
    ...
    @JvmStatic fun build(builder: BrowserListBuilder.() -> Unit): BrowserList = builder().apply(builder).build()

    /** @return Built [BrowserList] specification from the specified string or strings. */
    @JvmStatic fun browser(single: String): Browser {
      require(',' !in single) { "Single-browser token or expression must not contain a comma: $single" }
      return DefaultBrowser(single)
    }

    /** @return Built [BrowserList] specification from the specified string or strings. */
    @JvmStatic fun parse(suite: Sequence<String>): BrowserList = sequence {
      suite.forEach { token ->
        if (',' in token) {
    ...
    /** @return Built [BrowserList] specification from the specified string or strings. */
    @JvmStatic fun parse(first: String, vararg additional: String): BrowserList = sequence {
      yield(first)
