 ...
 */
package elide.tooling.project.manifest.flags
 ...
 */
@Serializable class ProjectFlagDefinition private constructor(
  private val info: FlagInfo,
) {
  // Private flag info structure.
  @Serializable @JvmRecord data class FlagInfo(
    val name: String,
  ...
  val aliases: List<String> get() = info.aliases
  val description: String? get() = info.description
  val type: ProjectFlagType get() = info.type
  val required: Boolean get() = info.required
  val defaultValue: ProjectFlagValue get() = info.defaultValue
